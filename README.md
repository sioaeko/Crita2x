...
| 레이어 | 빈 레이어 추가, 이름 변경, 복제/삭제, 표시 토글, 불투명도 조절, 블렌드 모드, 레이어 위치 이동, 레이어 마스크, 보이는 레이어 병합 |
...
Crita2x는 현재 활발히 개선 중입니다. 핵심 업스케일 대기열, 모델 탐색, 미리보기 조작, 누끼 도구, 선택 영역 편집, 레이어 이동, 레이어 마스크는 구현되어 있으며, 패키징과 설치 편의성, 더 세밀한 편집 워크플로는 계속 다듬고 있습니다.
