...
| 영역 | 기능 |
| --- | --- |
| 업스케일 | 드래그 앤 드롭 대기열, 선택 실행, 전체 실행, 중지, 최근 완료 결과 패널, 완료 결과 바로 보기, 탐색기 위치 열기, 배율/노이즈/타일/GPU/TTA/포맷 설정 |
...
| 레이어 | 빈 레이어 추가, 이름 변경, 복제/삭제, 순서 재정렬, 표시 토글, 불투명도 조절, 블렌드 모드, 선택 레이어 경계 표시, 레이어 위치 이동, 캔버스 핸들 직접 리사이즈, 캔버스 정렬, 레이어 크기/맞춤/회전/반전, 레이어 마스크, 보이는 레이어 병합 |
