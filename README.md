...
| 영역 | 기능 |
| --- | --- |
| 업스케일 | 드래그 앤 드롭 대기열, 선택 실행, 전체 실행, 중지, 상단 결과 보기/탐색기 버튼, 최근 완료 결과 패널, 배율/노이즈/타일/GPU/TTA/포맷 설정 |
| 모델 | 엔진 폴더의 모델 자동 탐색, CUNet/Anime/Photo 계열 선택, 외부 모델 폴더 지정 |
| 미리보기 | Ctrl+휠 확대/축소, Ctrl+드래그 이동, 격자 오버레이, 맞춤 보기, 100% 보기 |
| 비교 | 기준 이미지 저장, 현재 결과와 분할 비교, 비교 위치 슬라이더 |
| 레이어 | 썸네일 레이어 목록, 잠금/표시 토글, 빈 레이어 추가, 이름 변경, 복제/삭제, 순서 재정렬, 불투명도 조절, 12종 블렌드 모드, 선택 레이어 경계 표시, 레이어 위치 이동, 캔버스 핸들 직접 리사이즈, 캔버스 정렬, 레이어 크기/맞춤/회전/반전, 레이어 마스크, 보이는 레이어 병합 |
| 선택 영역 | 사각 선택, 자유형 올가미, 마술봉 색 영역 선택, 선택 마스크 오버레이, 페더/감도 조절, 선택 지우기, 선택만 남기기, 선택 영역 새 레이어 추출, 선택 영역 마스크화 |
| 클립보드 | 선택 영역 또는 현재 레이어 복사/잘라내기, 클립보드 이미지를 새 레이어로 붙여넣기, 이미지가 없을 때 클립보드로 새 캔버스 생성 |
| 누끼 | 테두리 기반 배경 제거, 크로마키, 색상 픽커, 페더, 디프린지, 투명 영역 트림 |
| 브러시 | 알파 지우개, 알파 복원, 브러시 크기/경도 조절, 주변 마스크를 참고하는 자동 복원 브러시, Alt+클릭 샘플 기반 복제 도장, 닷지/번 밝기 브러시, 마스크 숨김/복원 브러시 |
| 편집 | 회전, 뒤집기, 크롭, 긴 변 기준 리사이즈, 밝기/대비/채도/샤픈/노이즈 감소, HSL 색상 이동/채도/명도 보정, 히스토그램 기반 레벨 보정, 자동 보정 |
...
| `Ctrl+A`, `Ctrl+C`, `Ctrl+X`, `Ctrl+V` | 전체 선택, 선택/레이어 복사, 잘라내기, 붙여넣기 |
...
Crita2x는 현재 활발히 개선 중입니다. 핵심 업스케일 대기열, 모델 탐색, 미리보기 조작, 누끼 도구, 선택 영역 편집, 레이어 이동/정렬/변형, 레이어 마스크는 구현되어 있으며, 패키징과 설치 편의성, 더 세밀한 편집 워크플로는 계속 다듬고 있습니다.
